...
Agentrium includes built-in auto-update functionality using Tauri's updater plugin.
...
tauri signer generate -w ~/.tauri/agentrium.key
```

This creates:
- `~/.tauri/agentrium.key` - Private key (keep secret!)
- `~/.tauri/agentrium.key.pub` - Public key
...
```json
{
  "plugins": {
    "updater": {
      ...
      "endpoints": [
        "https://github.com/YOUR_USERNAME/agentrium/releases/latest/download/latest.json"
      ]
    }
  }
}
...
```json
{
  ...
  "platforms": {
    "windows-x86_64": {
      ...
      "url": "https://github.com/user/repo/releases/download/v1.2.0/Agentrium_1.2.0_x64-setup.nsis.zip"
    }
  }
}
```
